...
def df_to_longtable_md(df: pd.DataFrame, name: str, escape_cols=None):
    """Emit a page-breaking longtable instead of a fixed tabular.

    Needed for tables too tall to fit one page. A `tabular` inside a `table`
    float cannot break, so the overflow is silently clipped: S12 lost its last
    three rows and printed the page number inside the table body
    ("Float too large for page by 101.47pt").

    The caption goes inside the longtable, via a \\longtabcaption macro the caller
    defines just before \\input. longtable steps the table counter itself, so an
    external \\captionof would consume a second number and leave a gap.
    """
    esc = df.copy()
    if escape_cols:
        for c in escape_cols:
            esc[c] = esc[c].map(latex_escape)
    spec = "l" + "r" * (len(df.columns) - 1)
    head = " & ".join(str(c) for c in df.columns) + r" \\"
    ncol = len(df.columns)
    rows = [" & ".join(str(v) for v in rec) + r" \\"
            for rec in esc.astype(str).itertuples(index=False, name=None)]
    body = "\n".join([
        r"\begin{longtable}{" + spec + "}",
        r"\caption{\longtabcaption}\\",
        r"\toprule",
        head,
        r"\midrule",
        r"\endfirsthead",
        r"\multicolumn{" + str(ncol) + r"}{l}{\emph{Table \thetable\ continued}} \\",
        r"\toprule",
        head,
        r"\midrule",
        r"\endhead",
        r"\midrule",
        r"\multicolumn{" + str(len(df.columns)) + r"}{r}{\emph{continued on next page}} \\",
        r"\endfoot",
        r"\bottomrule",
        r"\endlastfoot",
        *rows,
...
def s12_sensitivity():
    ...
    # 54 rows: longtable, not tabular — see df_to_longtable_md.
    df_to_longtable_md(df.rename(columns={
...
def s16_equivalence_margins():
    ...
    LAB = {
        ...
        # Plain text here: df_to_latex_md escapes the Comparison column below.
        # Pre-escaping these made them render as "E4 transfer 0\{}% vs zero-shot".
...
def s_abbreviations():
    """Abbreviations and full forms, so a reader meeting a short form mid-table
    can resolve it without hunting back through the Methods."""
    rows = [
        ...
        ("NAS-GRU", "The GRU architecture discovered by NAS and used as the \\emph{search} tier"),
        ...
        ("PM$_{2.5}$", "Particulate matter with aerodynamic diameter $\\le$ 2.5 $\\mu$m"),
        ...
        ("$\\alpha$", "Significance level (0.05 throughout)"),
        ("$\\delta$", "Equivalence margin for TOST, in MASE units (0.05 in main-text claims)"),
        ("$\\lambda$", "Cost-penalty coefficient: MASE units traded per USD per 1,000 forecasts"),
        ("$m$", "Seasonal period of the naive baseline, in hours"),
    ]
    out = pd.DataFrame(rows, columns=["Abbreviation", "Full form"])
    body = out.to_latex(index=False, escape=False, column_format="ll")
    write("S_abbreviations", body, out.to_markdown(index=False))


def s_recipe():
    """Step-by-step recipe for applying the evaluation and decision rule to a new
    city, so the released code is usable rather than merely available."""
    rows = [
        ("1", "Gate the series",
         "Drop physically invalid readings, split at gaps $>$ 48 h, interpolate only "
         "gaps $\\le$ 6 h, and keep a window of $\\ge$ 2,160 h at $\\ge$ 60\\% real "
         "coverage. A city that fails this has too little history to compare "
         "strategies at all."),
        ("2", "Fix the MASE scale",
         "Compute the in-sample seasonal-naive ($m$ = 24) error on the pre-test "
         "history once per series and reuse it for every tier and every "
         "training-history regime, so scores stay comparable as history varies."),
        ("3", "Build causal covariates",
         "Enter each meteorological covariate at its last value known at the forecast "
         "origin; leave calendar features future-known. Use perfect-foresight "
         "covariates only as a labelled ceiling, never as the headline."),
        ("4", "Assemble the candidates",
         "At minimum: the zero-shot foundation model, one locally trained specialist, "
         "and the seasonal-naive floor. The foundation model needs no training; the "
         "specialist needs the covariate frame from step 3."),
        ("5", "Backtest",
         "Rolling-origin, six folds, all evaluated on the same final test window per "
         "series, so different training-history regimes meet on identical test data."),
        ("6", "Measure energy",
         "Wrap each tier's entire run and record joules per 1,000 forecasts. Repeat "
         "the workload five times and report any cell with sd/mean $>$ 20\\% as a "
         "range. Record training and inference separately."),
        ("7", "Set $\\lambda$",
         "Convert energy to cost with the local electricity price and PUE, then choose "
         "$\\lambda$, the MASE units worth one USD per 1,000 forecasts. Price and PUE "
         "rescale $\\lambda$ linearly, so a sweep covers a range of local conditions."),
        ("8", "Read the winner",
         "Select the tier minimizing MASE + $\\lambda \\cdot$ USD/1k in the cell "
         "matching the available training history. Check the neighbouring cells: a "
         "winner that flips under a small change in $\\lambda$ is not a robust choice."),
        ("9", "Check the retraining cadence",
         "Compare the planned refresh interval against the amortization crossover. "
         "Refresh more often than the crossover and the zero-shot model is cheaper; "
         "train once and serve for longer and the specialist wins."),
    ]
    out = pd.DataFrame(rows, columns=["Step", "Action", "What to do"])
    body = out.to_latex(index=False, escape=False,
                        column_format="rlp{0.62\\textwidth}")
    write("S_recipe", body, out.to_markdown(index=False))
