...
def pre(md, strip_heading=True):
    ...
    # Greek used in the equivalence/decision-rule prose. NB: "±" is deliberately not
    # remapped -- it already renders via inputenc/textcomp in the existing mean±sd text.
    md = md.replace("δ", "$\\delta$").replace("α", "$\\alpha$")
