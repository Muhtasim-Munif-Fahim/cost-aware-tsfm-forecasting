...
# What the manuscript's Code Availability statement actually promises is archived here:
#
...
#
# Four categories. Note "the per-claim results ledger" is paper/RESULTS_LEDGER.md, ONE
# document -- not the results/ output tree, and not the manuscript PDFs.
#
# This scope is deliberately NARROWER than the public GitHub repository. The two were
# briefly built from an identical file list so they "could not drift apart", but that
# conflated two different questions: GitHub answers "what is this project", and legitimately
# carries results/ and MANUSCRIPT/ under the CC BY terms in LICENSE-DATA-FIGURES.md; the
# deposit answers "what did the paper say is deposited here". Inheriting GitHub's scope put
# 1,211 result files and both manuscript PDFs into a citable archive that promised neither.
# Depositing the manuscript PDF in particular is a self-archiving decision for the authors
# to take deliberately, not a side effect of a shared allowlist.
DEPOSIT_SCOPE = [
    "src/*",                       # harness: run_forecast.py, e4_transfer.py, ...
    "analysis/*",                  # analysis scripts
    "figures/*.py", "figures/*.md",   # figure generation code
    "tables/*.py", "tables/out/*",    # table generation code + generated tables
    # Nothing from paper/ ships. ANALYSIS_PLAN.md and RESULTS_LEDGER.md were previously
    # promised by Code Availability, but they carry internal-process scaffolding
    # (automated-audit-script wording, "Phase N" x63, review-process wording) that does
    # not belong in a citable public archive, so the Code Availability, Methods and
    # Discussion wording was narrowed to the harness and figure/table code instead.
    # paper/METHODS.md was never promised at all and self-describes as a "skeleton"
    # superseded by the manuscript's own Methods section; it was included here by mistake.
    # repository metadata needed for the archive to stand on its own
    "LICENSE", "LICENSE-DATA-FIGURES.md", "README.md", "NAVIGATION.md",
    "CITATION.cff", "requirements.txt",
    "cities_manifest.csv", "cities_quality.csv",
    "data/manifest.sha256",        # SHA-256 input manifest named in Data Availability
]


def in_deposit_scope(path: str) -> bool:
    return any(fnmatch.fnmatchcase(path, pat) for pat in DEPOSIT_SCOPE)
...
def main():
    ...
    shipped, skipped_missing, refused, out_of_scope = [], [], [], []
    for rel in files:
        if blocked(rel):
            refused.append(rel)
            continue
        if not in_deposit_scope(rel):
    ...
    print(f"-> outside deposit scope: {len(out_of_scope)} "
          f"(on GitHub, but not promised by Code Availability)")
