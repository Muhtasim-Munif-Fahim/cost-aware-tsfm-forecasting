...
is additionally run with two further models, chosen so that the two possible explanations
are varied one at a time. `amazon/chronos-bolt-base` (205 M parameters) changes checkpoint
scale by 4.3-fold while holding the family, developer and pretraining corpus fixed.
`google/timesfm-2.5-200m-pytorch` (231 M parameters, decoder-only) changes family,
developer and pretraining corpus at a scale matched to Chronos-Bolt-base. Comparing the
three therefore isolates a scale effect within a family from a family effect at matched
scale, rather than confounding them. Everything else is held constant: the
same folds, the same four-week context window, the same metric, and the same
covariate-residual scheme for each model's covariate-consuming variant. TimesFM 2.5 provides a
native covariate interface, which is deliberately not used, because holding the covariate
pathway identical across families keeps the comparison about the foundation model rather
than about two different covariate implementations. None of the checkpoints is trained or
fine-tuned. Chronos-Bolt-small remains the tier carried through Table 1, the energy
measurements and the cost-adjusted decision rule; the other two enter as accuracy
generality checks, and we report where the three agree and where they do not.
...
Three properties of this construction are worth stating. First, the archive begins
2021-03-24, so only 15 of the 29 panel cities are covered, and the uncovered half is
disproportionately scarce-tier (9 of 14); calibrating on the covered cities and applying
the fitted error model to all 29 avoids running the experiment on a rich-city-biased
sub-panel. Second, the archive's `previous_day` fields carry lead times of 24--47 h whereas
our forecasts run at 1--24 h, so taking them at face value would overstate NWP error and
thereby bias the comparison toward the conclusion the causal ablation already supports; we
instead regress error on lead time across three lead days and evaluate at our own lead
band, reporting the measured anchors alongside the fitted value. Third, matching the error
*magnitude* of the last-known regime does not reproduce its effect. Equating error variance
predicts that the last-known regime should sit at $\alpha = 1.54$; the panel does not reach
it even at $\alpha = 3$ (0.731 against the regime's own 0.745), and linear extrapolation
places the equivalent near $\alpha \approx 3.4$, more than twice the variance-matched
prediction. Holding a covariate flat for 24 h removes the diurnal cycle, a coherent
distortion, whereas AR(1) noise of the same variance blurs that cycle but preserves it, so
persistence costs appreciably more than its variance alone implies. We therefore report the
last-known result directly rather than reading it off the $\alpha$ axis, and treat the
variance-matched equivalent as a lower bound on its cost.
...
load, and for trained tiers the per-fold training, as well as inference. This is why the
measured figure for a trained tier is not comparable with the inference-only proxy below.

The two hardware components are not on equal footing, and we separate them explicitly.
...
We accordingly interpret the CPU-side numbers as **comparative and directional**: they
support relative statements between tiers measured on this one machine under an identical
protocol, and they do not license claims about absolute hardware energy draw, nor
extrapolation to other processors. The GPU-side numbers, which dominate the
foundation-model and NAS tiers and therefore carry most of the headline energy contrast,
are NVML-measured and stand on firmer ground. Energy is normalized to joules per
1,000 forecasts and converted to USD at 0.15 USD/kWh with PUE 1.4 (defaults varied in
a sensitivity analysis, Supplementary Table S11). The supplementary TDP × latency proxy, by contrast, measures inference
only; the two are never averaged or ratioed against each other for trained tiers. Repeatability: a fixed workload was rerun five times on three cities
...
Those values are explicitly **not** treated as independent. Errors belonging to the same
...
original DM statistic assumes. The Bartlett weighting also guarantees a positive
semi-definite variance estimate, which matters at $h = 24$. The resulting statistic
additionally carries the Harvey–Leybourne–Newbold small-sample correction
\cite{harvey1997testing} and is referred to a Student $t(n-1)$ reference distribution
rather than a standard normal, because the HLN correction is derived to be paired with
$t(n-1)$; using the normal is a common shortcut that yields slightly anti-conservative
P values. Fold origins are non-overlapping, so the residual cross-fold dependence is that
of the series' own persistence, while within-trajectory and cross-horizon dependence is
absorbed by the HAC lag window. Significant-win counts are
additionally controlled with Benjamini–Hochberg FDR across the 29 cities.
...
contiguous blocks of the loss-differential series, so dependence *within* a block is
carried through the resample untouched rather than modelled; block length was swept over
12, 24 and 48 steps (half a forecast trajectory, exactly one, and two) because a block
bootstrap's validity rests on the block spanning the dependence range, and sweeping it
tests that rather than assuming it. A trajectory-level test then collapses each 24-step
trajectory to its mean, leaving one value per non-overlapping fold origin ($n = 6$) and
discarding within-trajectory and cross-lead dependence entirely instead of correcting for
it. The bootstrap returns slightly *more* significant cities than the HAC test in both
domains (PM2.5 4–8 versus 2 of 29; temperature 10–12 versus 8 of 29; Supplementary
Table S17), so the parametric
correction is conservative rather than permissive on these data, and the deliberately
underpowered trajectory-level test returns fewer (1 and 3 of 29), as its $n = 6$ implies.
...
Two conventions are worth stating explicitly, because they put several different
numbers close together. First, δ and α are distinct quantities that happen to share
the value 0.05: δ = 0.05 is a margin measured in MASE units, whereas α = 0.05 is a
significance level. Second, the interval TOST tests against is the $1-2\alpha = 90\%$
confidence interval, not the 95% one. Equivalence is declared when that 90% interval
lies entirely inside the margin. This is why every TOST interval reported here is a 90%
interval, while the accompanying 10,000-resample bootstrap CI on the paired difference
is a conventional 95% one.
