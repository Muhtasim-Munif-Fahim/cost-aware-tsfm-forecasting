...
| File | What |
|------|------|
| [`paper/RESULTS_LEDGER.md`](paper/RESULTS_LEDGER.md) | Per-claim ledger: every reported number, its artifact and producing command |
| [`README.md`](README.md) | Repository overview and headline results |

> The manuscript, its LaTeX source and the journal template are not published here.
> This repository carries the code and results; the paper is available from the
> publisher, or from the authors on request.
